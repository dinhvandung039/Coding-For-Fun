...
void AddMemberToSimulation(SharedStateFactory factory, NeighborhoodMember member, int i)
{
...
Subdivision GetRandomSubdivision(Random rng, IReadOnlyList<Subdivision> subdivisions)
{
    var randomIndex = rng.Next(subdivisions.Count);
    return subdivisions[randomIndex];
}

TransportationMethod GetRandomTransportation(Random rng, IReadOnlyList<TransportationMethod> transportMethods)
{
    var randomIndex = rng.Next(transportMethods.Count);
    return transportMethods[randomIndex];
}

Tuple<int, int> GetRandomCoordinates(Random rng, int limit)
{
